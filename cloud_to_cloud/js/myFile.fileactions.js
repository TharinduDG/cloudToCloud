	...
	display: function (parent) {
		...
		var addAction = function (name, action) {
			...
			if (name === 'Download' ) {
				...
				element.on('click', {a: null, elem: parent, actionFunc: actions[name]}, function(e){
                    e.stopPropagation();
                    e.preventDefault();
                    var fieId = $(this).parent().parent().parent().parent().attr('data-id');

                    $.ajax({
                        type:"POST",
                        url:OC.filePath('cloud_to_cloud', 'ajax', 'transfer.php'),
                        data:{dataId:fieId},
                        dataType:"json",
